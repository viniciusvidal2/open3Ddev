...
## Ler nuvem
path = "C:/Users/vinic/Desktop/Texturization_Alice"
cloud = o3d.io.read_point_cloud(os.path.join(path, "merged.ply"))
cl, ind = cloud.remove_statistical_outlier(nb_neighbors=100, std_ratio=1.5)
cloud2 = cloud.select_by_index(ind, invert=True)
cloud2.paint_uniform_color([1.0, 0, 0])
cloudv = cloud.voxel_down_sample(voxel_size = 0.005)
...
mesh, densities = o3d.geometry.TriangleMesh.create_from_point_cloud_poisson(not_plane, depth=12)
## Filtrar por densidades
densities = np.asarray(densities)
densities = (densities - densities.min()) / (densities.max() - densities.min())
vertices_to_remove = densities < np.quantile(densities, 0.1)
...
o3d.io.write_triangle_mesh(os.path.join(path, "mesh.ply"), mesh_final)
